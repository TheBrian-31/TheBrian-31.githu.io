...
import { RiDeleteBinLine, RiEditLine } from "react-icons/ri"; // Asegúrate de importar RiEditLine
import defectImage from '../../assets/images/Userdefault.png';

const PeopleCard = ({ username, DUI, Email, image, onDelete, onEdit, onView }) => {
    return (
        <div className="flex flex-row justify-center w-full border border-black rounded p-4 bg-white">
            <div className="flex flex-col md:flex-row w-full">
                ...
                <div className="flex flex-col justify-normal md:justify-center md:ml-3 flex-grow max-w-xs md:max-w-sm lg:max-w-md overflow-auto">
                    <p className="m-0 text-sm md:text-lg lg:text-lg font-bold">{username}</p>
                    <p className="m-0 text-sm md:text-lg lg:text-lg">DUI: {DUI}</p>
                    <p className="m-0 text-sm md:text-lg lg:text-lg">Email: {Email}</p>
                </div>
            </div>
            <div className="flex flex-col ml-auto items-center">
                ...
                {onEdit && (
                    <button onClick={onEdit} className="flex items-center justify-center text-white bg-blue-500 border rounded p-1 md:p-2 m-1">
                        <RiEditLine className="text-lg md:text-xl" />
                    </button>
                )}
            </div>
        </div>
